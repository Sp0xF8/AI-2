...
bool CheckPlato(){
	...
	if(g > 10){
		if( Helper::average_fitnesses[g] < Helper::average_fitnesses[g - 10] + PLATO_HEIGHT 
											&& 
			Helper::average_fitnesses[g] > Helper::average_fitnesses[g - 10] - PLATO_HEIGHT
			){
			return true;
		}
	}
		...
		for ( mutation_rate = 0.02; mutation_rate < 0.3; mutation_rate += 0.01) {
			for ( mutation_height = 0.04; mutation_height < 0.2; mutation_height += 0.01) {
				...
				for (g = 0; g < NUMBER_OF_GENERATIONS; g++) {
					...
					current_data.generation = Helper::meta_data.size();
					...
					current_data.solution_fitness = Helper::GetSolutionFitness(current_data);
